...
def ImproveBorderline(T, M=10, K=5):
    """
    {T}: 总样本
    {P}: 少数类
    {N}: 多数类
    ...
    if c < 1:
        # R = random.sample(DANGER, math.floor(c * dnum))
    ...
    for i in range(R_len):
        nnarray = neigh_K.kneighbors([P_array[R[i]]], K + 1, return_distance=False)
        ...
        for item in nnarray_r:
            dif = np.array(P_array[item]) - np.array(P_array[R[i]])
